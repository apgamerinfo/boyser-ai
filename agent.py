...
# ---------- เช็คอัปเดตจาก GitHub ----------

def check_update_bg() -> None:
    """เช็คเวอร์ชันใหม่จาก git remote ใน daemon thread (เงียบ ไม่ถ่วง startup)
    เตือนเฉพาะตอน remote ใหม่กว่าจริง — เครื่อง dev ที่ commit ล้ำหน้า remote ไม่เตือน"""

    def run():
        try:
            if not os.path.isdir(os.path.join(REPO_DIR, ".git")):
                return  # ติดตั้งแบบไม่มี git (เช่น โหลด zip) → เช็คไม่ได้
            ...
            if not local or not remote or local == remote:
                return
            # remote เป็นบรรพบุรุษของ HEAD = เราล้ำหน้าเอง ไม่ใช่ตกรุ่น; ถ้า object ไม่รู้จัก = remote มีของใหม่
            anc = subprocess.run(
            ...
            pass  # ออฟไลน์/ไม่มี git → เงียบไว้
...
def main() -> None:
    ...
    while True:
        if UPDATE_AVAILABLE.is_set():
            UPDATE_AVAILABLE.clear()  # เตือนครั้งเดียวพอ
        ...
        if user == "/update":
            ...
            if "Already up to date" in out:
                ...
                continue
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "-r", os.path.join(REPO_DIR, "requirements.txt"), "-q"],
                capture_output=True,
            )
            # ติดตั้ง skills ใหม่ที่เพิ่มมากับเวอร์ชันนี้ (เฉพาะตัวที่ยังไม่มี — ไม่ทับของที่ user แก้เอง)
